  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in text-left">
      <div className="max-w-md w-full rounded-2xl border border-white/10 bg-[#0e0a1c] p-6 glass-strong shadow-2xl space-y-5 relative">
        ...
        <div>
          <h3 className="text-sm font-semibold text-white">Edit Document Name</h3>
        ...
        <div className="space-y-1.5">
          ...
          <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full h-9 bg-black/40 border border-white/10 rounded-lg px-3 text-xs text-white focus:outline-none focus:border-[var(--accent)]/50 transition-colors" placeholder={fileName} />
        </div>
