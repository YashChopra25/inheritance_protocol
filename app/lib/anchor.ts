...
/** Per-(token vault, beneficiary) claim marker — mirrors the on-chain seeds. */
export function tokenClaimPda(
  tokenVault: PublicKey,
  beneficiary: PublicKey
): PublicKey {
  return PublicKey.findProgramAddressSync(
    [SEEDS.TOKEN_CLAIM, tokenVault.toBuffer(), beneficiary.toBuffer()],
    PROGRAM_ID
  )[0];
}
...
/** Token-2022. The program accepts it via `TokenInterface`; the client must too. */
...
/**
 * M7: the on-chain program takes any `TokenInterface` mint, but the client
 * previously hardcoded the legacy program here — so every Token-2022 mint
 * derived the wrong ATA and failed. Always pass the mint's real owner program,
 * which `resolveTokenProgram` reads from the chain.
 */
...
/**
 * Which token program owns this mint. A mint account's owner IS its token
 * program, so this is a single account read rather than a guess.
 */
export async function resolveTokenProgram(
  ...
  if (
    ...
    throw new Error(`${mint.toBase58()} is not an SPL token mint`);
  }
  return info.owner;
}
 ...
 *
 * H5: the length guard here used to be commented out. Without it,
 * `bytes.forEach((b, i) => out[i] = b)` grew the array past `len`, so an
 * oversize value no longer matched the contract's `[u8; N]` and Anchor either
 * threw at serialization or silently wrote the wrong shape. The guard is back,
 * and callers that legitimately need truncation ask for it explicitly rather
 * than getting it by accident.
 */
export function strToFixedBytes(
  value: string,
  len: number,
  ...
  if (bytes.length > len) {
    if (!truncate) {
      throw new Error(
        `Value "${value}" encodes to ${bytes.length} bytes; the on-chain field holds ${len}`
      );
    }
...
/** MIME types are advisory metadata, so an unusually long one is truncated
 *  rather than blocking an upload the user cannot otherwise complete. */
export const MEDIA_TYPE_BYTE_LEN = 16;

export function mediaTypeToBytes(mime: string): number[] {
  return strToFixedBytes(mime, MEDIA_TYPE_BYTE_LEN, { truncate: true });
}
